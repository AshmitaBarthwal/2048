...
#Intialising all features of pygame we need
pygame.init()

#Frames Per Second-Regulates speed on different devices and tells how quickly the game is running
FPS = 60
...
MOVE_VEL = 20    #speed with which tile move

#Creating Pygame Window where we draw objects
WINDOW = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("2048")

#Defining Tiles
class Tile:
    #each tile has different color based on it's value
    COLORS = [
        (237, 229, 218), # 2
        (238, 225, 201), # 4
        (243, 178, 122), # 8
    ...
    #Initialisation Of Tile- constructor takes the value, row and col of the tile
    ...
    def get_color(self):
        ...
        #log2(2)=1 but its at index 0 so we subtract 1 from it to make it 0
        #similarily, log2(4)-1 = 2-1 = 1, and at index 1 of COLORS list we have color for the number 4
    ...
    def draw(self, window):
        ...
        #blit will put the text surface onto the screen
        window.blit(
            text, 
            #Formula to get the text in the middle of the tile
            (
    ...
    #sets the row and col of a tile based on its x and y values
    ...
    def move(self, delta):
        self.x += delta[0]   # delta-How much we want to move a tile
        self.y += delta[1]

#function to draw Horizontal And Vertical Lines Inside the Grid
...
def draw(window, tiles):
    ...
    #to draw our tiles, we will use a dictionary to locate tiles quickly
...
#function to generate random row and random column to place the tile
#We will do this only for the tile that do not exist
...
def move_tiles(window, tiles, clock, direction):
    ...
    #blocks tells us which tile has already merged
    #so that we dont merge them again
    #e.g. 2 2 2 2
    #according to the rules of the game we should get 4 4
    #but if we dont use blocks we might get wrong value like 8
    blocks = set()
    ...
    if direction == "left":
        ...
        #Determines the order in which we sort the column
        reverse = False 
        ...
        ceil = True
    #Now for other directions we do the same process
    ...
    #while loop will run atmost 1 time in 60 seconds
    #clock regulates speed of the loop
    while updated:
        clock.tick(FPS)
        ...
        #enumerate over the sorted tiles and get the index of tile as well as the tile object
        for i, tile in enumerate(sorted_tiles):
            ...
            #we do this by checking if they exist in blocks set or not
                ...
                if merge_check(tile, next_tile):
                    ...
                    #we now remove the tile at index i that has merged with the next tile
                    sorted_tiles.pop(i)
            ...
            #look at the current position of this tile and
            # adjust the row and column based on the x and y position
            tile.set_pos(ceil)
            updated = True
...
def update_tiles(window, tiles, sorted_tiles):
    ...
    #Copy all the tiles that are left in sorted_list
...
#THIS IS DONE WHEN GAME GETS STARTED
def generate_tiles():
    tiles = {}
    for _ in range(2):
        #get_random_pos will generate the random positions
        #for example "00":Tile(4,0,0) tells that 
        #x coordinate is 0 and y coordinate is 0
        #and the text written on the tile is 4
        #we used fstring here and any value inside it will be converted to string
        row, col = get_random_pos(tiles)
...
def main(window):
    ...
    #GAME LOOP 
    #this loop checks for button presses,exiting screen and
    #running the game
    while run:
...
#main(WINDOW)- we want to run the game on windows
#main function will get executed if we run the file directly
